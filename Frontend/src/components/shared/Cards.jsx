...
const Card = ({ id,image, title, subtitle, type ,track }) => {
  const {isPlaying,selectTrack, setMusicMode}=useUserStore()
  ...
  return (
    <div key={id} className="w-48 p-4 bg-[#121212] hover:bg-[#1f1f1f] rounded-lg transition-colors duration-300 group cursor-pointer select-none">
      ...
      <div className=" overflow-hidden">
        ...
        <p className="text-[#a7a7a7] text-sm truncate font-spotify-circular ">
          {subtitle}
        </p>
      </div>
    </div>
